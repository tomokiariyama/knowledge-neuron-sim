...
class Patch(torch.nn.Module):
    ...
    def __init__(
        ...
        assert mode in ["replace", "suppress", "enhance", "suppress_by_replace"]
        self.mode = mode
    ...
    def forward(self, x: torch.Tensor):
        ...
        elif self.mode == "suppress_by_replace":
            for pos in self.target_positions:
                x[:, self.mask_idx, pos] = self.acts[pos]
        else:
        ...
        for layer_idx, positions in neurons_dict.items():
            ...
            set_attribute_recursive(
                ...
                Patch(
                    ff_layer,
                    mask_idx,
                    replacement_activations=replacement_activations[layer_idx],
                    mode=mode,
                    target_positions=positions,
                ),
            )
